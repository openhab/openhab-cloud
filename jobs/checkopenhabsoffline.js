...
module.exports = new cronJob('00 00 00 * * *', function () {
    logger.info('checkopenhabsoffline job started');
    date3DaysAgo = new Date;
    date3DaysAgo.setDate(date3DaysAgo.getDate()-3);
    logger.info('date3DaysAgo = ' + date3DaysAgo);
    Openhab.find({status:'offline', last_online: {'$lt':date3DaysAgo}}, function (error, openhabs) {
        if (error) {
            logger.error('Error finding offline openHABs: ' + error);
        }

        if (!openhabs) {
            logger.info('No offline openHABs found');
        }
        logger.info('Found ' + openhabs.length + ' openhabs');
        for (var i in openhabs) {
            ...
            UserAccount.findOne({_id:openhab.account}, function (error, userAccount) {
                if (error) {
                    logger.error('Error finding user account for openhab: ' + error);
                }

                if (!userAccount) {
                    logger.error('Unable to find user account for openhab which is nonsense');
                }
    ...
    logger.info('checkopenhabsoffline job finished');
});
